...
from utils import load_config, set_up_experiment, load_data, log2p1, exp2m1, split_sk_params, setup_param_dist
...
param_dist = setup_param_dist(param_grid)
sampler = ParameterSampler(param_dist, random_state=9)
...
for idx, (outer_train_index, outer_test_index) in enumerate(outer_kf.split(X)):
    ...
    combo_rmses = []
    for idx_2, combo in enumerate(sampler):
        if idx_2 % 2 == 0:
            print("param combo nr. ", idx_2, "/", len(sampler))
        rmses = []
