...
def compute_metrics(y_test, y_pred):
    RMSE = cp.sqrt(mean_squared_error(y_test, y_pred))

    if len(cp.unique(y_test))==1:
        print("pearsonr undefined: all test samples are equal")
        pearson_corr=float('NaN')
    elif len(cp.unique(y_pred))==1:
        print("pearsonr undefined, all predicted samples are equal, value: "+str(y_pred[0]))
        pearson_corr=float('NaN')
    else:
        pearson_corr = cp.corrcoef(y_test, y_pred)[0,1]
...
def plot_training_results(y_test, y_pred, exp_dir):
    ...
    axs[1].scatter(cp.arange(len(y_test)), y_test)
    axs[1].scatter(cp.arange(len(y_pred)), y_pred)
...
def log2p1(X):
    if isinstance(X, pd.Series):
        ...
        raise TypeError("Wrong data type, must be pd.Series or cupy array.")
    return logged
